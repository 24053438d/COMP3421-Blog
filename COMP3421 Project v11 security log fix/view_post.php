...
// Process comment actions (approve/delete)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['comment_action']) && isset($_POST['comment_id'])) {
...
// Get pending comments if user is admin or post author
$pending_comments = [];
if (isset($_SESSION['user_id']) && 
    ($_SESSION['user_id'] == $post_data['user_id'] || 
     (isset($_SESSION['role']) && $_SESSION['role'] === 'admin'))) {
...
<div class="row">
    <div class="col-md-8 offset-md-2">
        ...
        <section class="comments-section">
            ...
            <!-- Pending comments section for post author or admin -->
            <?php if (!empty($pending_comments)): ?>
                <div class="mb-4">
                    <h4>Comments Pending Approval (<?php echo count($pending_comments); ?>)</h4>
                    <div class="list-group">
                        <?php foreach ($pending_comments as $pending_comment): ?>
                            <div class="list-group-item">
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    <strong><?php echo htmlspecialchars($pending_comment['username']); ?></strong>
                                    <small class="text-muted"><?php echo date('M d, Y g:i a', strtotime($pending_comment['created_at'])); ?></small>
                                </div>
                                <p class="mb-1"><?php echo nl2br(htmlspecialchars($pending_comment['content'])); ?></p>
                                <div class="mt-2">
                                    <form method="POST" style="display:inline-block;">
                                        <input type="hidden" name="comment_id" value="<?php echo $pending_comment['id']; ?>">
                                        <button type="submit" name="comment_action" value="approve" class="btn btn-sm btn-success">Approve</button>
                                        <button type="submit" name="comment_action" value="delete" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete this comment?')">Delete</button>
                                    </form>
                                </div>
                            </div>
                        <?php endforeach; ?>
